...
Write the report to COPILOT_REVIEW_PR{pr_number}.md."""
...
def main():
    parser = argparse.ArgumentParser(description="PR Review Script using GitHub Copilot")
    ...
    parser.add_argument(
        "--work-dir",
        help="Directory within the worktree where copilot should run (default: worktree root)",
    )
    parser.add_argument(
        "--output-dir",
    ...
    print(f"Reviewing PR #{pr_number} with GitHub Copilot ({args.model})...")
    ...
    # Step 3: Execute Review in Separate GitHub Copilot Process
    print("Step 3: Executing review in separate GitHub Copilot process (background mode)...")
    review_path = worktree_dir / args.work_dir if args.work_dir else worktree_dir

    print(f"Running GitHub Copilot review in {review_path}...")
    ...
    formatted_prompt = selected_prompt.format(pr_number=pr_number)

    # Run copilot CLI command in prompt mode
    copilot_cmd = [
        "copilot",
        "-p",
        formatted_prompt,
